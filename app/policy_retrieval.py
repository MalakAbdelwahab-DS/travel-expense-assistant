...
class PolicyRetrieval:
    ...
    def _answer_with_llm(self, question: str) -> list[dict]:
        """Real model path: ask the AI agent."""
        agent = get_agent()
        result = agent.invoke({"messages": [{"role": "user", "content": question}]})
        return result["messages"][-1].content_blocks[0]["text"]

    def _answer_with_retrieval(self, question: str) -> list[dict]:
        """Offline path: look the answer up directly in the CSV, no LLM involved."""
        words = question.lower().split()

        matches = self.data[self.data.apply(
            lambda row: any(w in " ".join(str(v).lower() for v in row) for w in words),
            axis=1
        )]

        if matches.empty:
            text = "Sorry, I cannot answer your question as it isn't covered by the travel expense policy."
        else:
            rows = matches.astype(str).agg(" | ".join, axis=1)
            text = "Based on the travel expense policy:\n" + "\n".join(rows)
